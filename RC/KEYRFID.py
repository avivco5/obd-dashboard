...
def search_barcode_by_description():
    ...
    for barcode, item in inventory.items():
        if term in item["description"].lower():
            matches.append((barcode, item["description"], item["quantity"]))
    ...
    if not matches:
        ...
        # פתיחת חלון חדש עם טקסט הניתן להעתקה
        ...
        for barcode, desc, qty in matches:
            text_box.insert("end", f"מקט: {barcode} | תיאור: {desc} | כמות: {qty}\n")
...
"""
tk.Label(user_frame, text="🔍 חפש לפי תיאור:").pack()
