...
def update_inventory_display():
    inventory_text = "📋 מלאי נוכחי:\n"
    for barcode, item in inventory.items():
        inventory_text += f"{item['description']} ({barcode}): {item['quantity']} יח'\n"
    inventory_display.set(inventory_text)
...
def pull_item():
    ...
    messagebox.showinfo("בוצע", f"הוצאה של {qty} יחידות מהפריט: {inventory[barcode]['description']}\n"
                                f"כמות קודמת: {prev_qty} → כמות חדשה: {new_qty}")
...
inventory_display = tk.StringVar()
