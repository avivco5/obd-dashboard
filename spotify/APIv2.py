...
def download():
    ...
    [YT_DLP_PATH, "--cookies-from-browser", "chrome", "--flat-playlist", "--dump-json",
     "--playlist-start", "1", "--playlist-end", "1", url],
    capture_output=True, text=True, check=True
        ...
        # Fetch all videos
        result = subprocess.run(
            [YT_DLP_PATH, "--cookies", COOKIES_FILE, "--flat-playlist", "--dump-json", url],
            capture_output=True, text=True, check=True
        )
        ...
        for video in videos:
            ...
            try:
                ...
                if "Sign in to confirm you" in result.stderr:
                    ...
                    wait_for_cookies_update(COOKIES_FILE)
                    continue
